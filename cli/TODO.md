...
The seed reference lives in `seeds/rewritable.html` (grep `function replaceDocument`, `function extractFrozenZones`, `function lockedRangesIn`, `function markerZoneRangesIn` — line numbers drift, so the CLI mirror comments cite function names, not offsets). **Still scoped down vs the seed's `replaceDocument`** (none are silent — each is a documented divergence): `parse_error_post_apply` (the CLI is parser-free, no DOMParser), `MAX_DOC`/`target_size_exceeded` (whole-doc size cap, also open on the apply-edits path below), and `canonLF` normalization of `currentDoc`/`newDoc` before the checks (a CRLF-only difference in a frozen zone could read as drift in the CLI but not the browser). The marker-form frozen-zone integrity (3 fence forms, unterminated, duplicate, byte-preservation, add-rejection), attribute-form `data-rwa-frozen`, class-lock coverage, reserved-id, lone-surrogate, and rwa-id-strict checks ARE mirrored.

Two regex limits are inherited from the seed verbatim (parity is intact — the browser enforces the real semantics via the DOM, the CLI/seed text-scan is the offline approximation, and "fixing" the CLI would DIVERGE from the seed): `lockedRangesIn` matches only quoted `class="…"` attributes (an unquoted `class=rwa-locked` is not detected), and `matchingCloseEnd` exempts self-closing same-tag opens from nesting where the seed's `findCloseTagEnd` counts them (a `<div/>` inside a locked `<div>` yields different ranges). If the seed tightens either, re-mirror.
...
- **Class-lock checks** — `class_lock_uncovered` is now enforced on the `replace_document`/escape path (2026-06-09; see the `edit.mjs` section above). `class_lock_violation` (an `apply_edits` find-range crossing a `.rwa-locked` subtree) is **STILL OPEN** — needs `lockedRangesIn` (now exported) wired into the per-edit loop in `applyEdits`, mirroring the seed's apply path (~:4473).
