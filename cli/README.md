...
`--reference <file>` (repeatable) bundles a markdown reference **inside the signed record** — Agent Skills' progressive disclosure, carried by the file itself so a recipient gets the bytes rather than a link that may be gone. A carrier with references is an `rwa-agent/2`; one without stays `rwa-agent/1`, so nothing about existing carriers changes.

Two things worth knowing. References are signed, so a tampered one fails verification exactly as a tampered prompt does, and an unverified record never releases its reference bytes. And they live inside the container, so they count against its own document budget (1 MB) — limits are `16` files / `64 KB` total, enforced here at authoring rather than surfacing later as a baffling `target_size_exceeded` on an unrelated edit. `rwa doctor` reports the share they take.
