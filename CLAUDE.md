...
- Product-kind machinery is **CLI-only** (added 2026-05-18 as R9-minimal). `rwa new --kind <name>` swaps the `INLINE_DOC` body and the lens placeholder at emit time via `kindOverrides(kind)` in `cli/src/seed.mjs`. The substrate is unaware of kinds — `SYSTEM_PROMPT` is intentionally not varied here (that's audit R1's job, parameterize-the-prompt-registry). Known kinds: `document` (default), `workflow`. Adding a kind = one entry in `KIND_TABLE` (`cli/src/seed.mjs`) plus a line in `cli/README.md` and the help text in `cli/bin/rwa.mjs`. The lens-placeholder regex anchors on the stable `id="rwa-lens-input" rows="N"` prefix; a copy change to the default placeholder won't silently break substitution — `applySeedSubs` enforces exactly-one match.
