...
- `service/public/import.html` ports three pieces of `cli/src/seed.mjs` logic: `escapeTL`, the INLINE_DOC backtick-walk, and the TITLE/FILE substitutions (it does **not** mirror the DOC_UUID substitution — that's server-side via `/rewritable.html`). It also ports `convertCsv` from `cli/src/import.mjs` for the CSV path. If any of those change on the CLI side, mirror the change here too.
