...
- `service/` — Node HTTP service that hands out fresh containers via `/new`, converts user-supplied documents into a container via `/import` (browser-side conversion against the same seed endpoint; supports md/csv/docx/pdf), and hosts published-snapshot shares via `POST /publish` → `GET /s/<short>` (anonymous, 24h expiry, 10 publishes/hour per IP, 25 MB body cap). Build context is the **repo root** (so it can `COPY seeds/`); see `service/Dockerfile` and `service/docker-compose*.yml`. Snapshot storage lives in `service/data/` (named volume `rwa_shares` in prod, bind-mounted `./data` in dev) — operator-readable, never in version control.
