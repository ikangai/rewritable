...
- **Runtime-owned region commit** → `docs/specs/rwa-runtime-region-commit-spec.md` (the `runtimeRegionCommit` primitive) AND, once implemented, `seeds/rewritable.html` (`runtimeRegionCommit` beside `synthesizeAndCommit`; `commitCore`/`replaceDocument` gain an optional `frozenBypass` param — additive to the R5 path). Builds on R5 (`docs/plans/2026-05-30-r5-write-path-design.md`). Consumers call it; they don't reimplement it — skill persistence (actions-v0.8 §7, `buildSkillZone`, `reachability:'frozen'`) and skinning-v2 L1 (`reachability:'edit-reachable'`).
