...
- Workflow v0.10 — `ctx.iter.parent` chain (added 2026-05-19). Nested foreach iterations expose an upward chain to outer iters via `ctx.iter.parent`, `ctx.iter.parent.parent`, etc. Top-level foreach has `ctx.iter.parent === undefined`. Same shape as `ctx.iter`: `{index, item, total, parent}`. The runner already set the field in v0.4 (`parent: ctx.iter || undefined`); v0.10 promotes from spec-reserved to spec-shipped. Spec: §3.2 iteration scope; §4 Ctx interface gains `IterCtx` type. Conformance: `benchmark/scenarios/conformance/workflow-17.mjs` (outer + inner + grandparent-undefined assertions across 3 leaf executions).
- Workflow v0.9 — multi-row parallel (added 2026-05-19). The largest deferred item from earlier non-goals. A `<table class="rwa-parallel">` may now contain multiple `<tr>` rows in `<tbody>`; each table column becomes an independent sequential pipeline, columns run via `Promise.all`. Constraints (validated before any cell runs): rows must all have the same cell count (`parallel_row_mismatch`), and cells in the same column must share `data-rwa-label` (`parallel_label_mismatch`). Column labels still must be unique across columns. Each column's first cell receives the parallel block's upstream `prev`; subsequent cells in the column receive the previous cell's return value. Output object shape unchanged: `{ [colLabel]: column.lastReturn, … }`. Cell-level `data-allow-failure` (v0.6) now applies within a column: a tolerated failure substitutes an `{__error,__code}` object as the next cell's `prev`. Implementation: new `parallelColumns(table)` helper does extraction + validation, returns `{label, cells[]}[]`; `runParallel` rewritten around it. Single-row workflows continue to work unchanged (one cell per column). Spec §3.3 "Multi-row case" subsection. Conformance: `benchmark/scenarios/conformance/workflow-{15,16}.mjs`.
- Workflow v0.8 — container dirty/stale tracking + session-state cache (added 2026-05-19). Containers (foreach, parallel table) now carry `data-last-run-hash` like leaves. The hash is computed via a recursive `nodeFingerprint`: foreach = `foreach:` + the join of inner-node fingerprints; parallel = `parallel:` + each cell's `label=fingerprint/F-or-A` (A = `data-allow-failure="true"`) in DOM order; leaf = its script body. Mismatch with stored ⇒ runner marks the container `.stale`. Editing any inner step body, adding/removing/reordering inner nodes, or toggling a cell's allow-failure flag flips the container `.stale` automatically. Bundled fix for a pre-existing v0.3 gap: runner state (`data-last-output`, `data-last-run-hash`) was DOM-only and got wiped by every `applyEnvelope` re-render. v0.8 adds a `window.__rwa_workflow_session` Map cache keyed by `data-rwa-id`; `persistLastRunHash` and `cacheOutput` write to both DOM and cache, `restoreSessionState` runs at the top of every `attachGestures` cycle to repopulate DOM from cache. Stale detection now survives lens edits cleanly. Lost only on full reload. Spec: §5 attributes table extended; §5.1 container-staleness paragraph rewritten; §8 non-goals shrinks. Conformance: `benchmark/scenarios/conformance/workflow-14.mjs`.
- Workflow v0.7 — container test-step (added 2026-05-19). The ▶ button now also attaches to foreach and parallel containers. Clicking it runs JUST that container's subtree against the upstream's cached value via the same `runForeach` / `runParallel` paths the full Run uses. Implementation: `testStep` dispatches on node type; for containers it calls a new `testContainer` helper that resolves upstream cache, clears the subtree's classes/outputs, and invokes `runNode`. Sibling top-level steps stay untouched (their `.done` class and cached outputs from a previous Run persist). Spec: §7 runner contract gains a per-container test bullet; §8 non-goals removes container-level test-step. Conformance: `benchmark/scenarios/conformance/workflow-13.mjs` verifies inner=3 calls, source=0, downstream=0 when testing a foreach.
...
- Substrate 0.11 — `TABLE` and `TD` joined `ANCHORABLE_TAGS` (added 2026-05-19). The DOM walker + source scanner in the bootstrap now descend INTO `<table>` rather than skipping past it (outer-wins exception, matched in both `buildSourcePositionMap` and `injectMissingBlockIds`). Result: parallel blocks and cells auto-receive `data-rwa-id` on bootstrap + every commit, exactly like other anchorable blocks. Closes the v0.5 caveat — pin works on parallel tables without authors needing to populate the id by hand. Pin button still defensively disables when the id is genuinely missing (legacy doc, ⌘S to populate). Spec: `re-write-able-spec.md` §5.9 + version 0.11 stanza.
