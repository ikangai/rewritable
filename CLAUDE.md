...
- **Artifact drop bus (typed droppables / "drop X onto a rewritable" / classifier / dispatcher / `accepts`)** → `seeds/rewritable.html` (`classifyArtifact`/`dispatchArtifact`/`readArtifactDeclaration`/`resolveAccepts`/`routeInstall`+`routeInstallFromText`, beside `handleCarrierDrop`) AND `docs/plans/2026-07-06-artifact-bus-design.md` (`rwa-artifact/1` v1 design) + `docs/plans/2026-07-06-artifact-bus-plan.md` (5-increment TDD plan). The bus **generalizes the drop gesture** into a small closed set of typed classes, each keeping its own trust model — the bus COMPOSES existing machinery, never reimplements or weakens it. One `classifyArtifact(input)` → `{class, source, payload}` over both drop paths: a DECLARED branch (an `rwa-artifact/1` compose tag, then the unchanged `classifyInstallText` carrier/envelope sniff) that BEATS a SNIFF branch (an image `file.type`) — a self-declared artifact is never silently re-typed; a recognized-but-unhonorable envelope FAILS LOUD to all-null. Three v1 classes: **install** (AI/skill carrier or bare envelope → `routeInstall` consent dialog, any mode, window-wide), **ingest** (image → `insertImageFiles`, Edit-mode-gated + mount-targeted, actor `user:image-drop`), **compose** (a dropped skin `rwa-artifact/1` tag → `applySkinL1`, one ⌘Z, actor `skin:NAME`, no mode gate — matches `/skin`/✦). The two drop handlers consult the classifier: `handleCarrierDrop` (capture-phase `window` drop — claims an `.html`/`.htm` SYNCHRONOUSLY by file-sniff, then routes by content) and `handleMountDrop` (Edit-mode mount, passes `{mode,target}`). The **`accepts` declaration** (design §3): `resolveAccepts()` reads an `accepts:[<class-name>,…]` array + optional doc-level `strict` boolean from the **edit-unreachable** `#rwa-affordances` (forgery-guarded via `isEditUnreachable` — a driftable/lens-written declaration is IGNORED); default = accept-all advisory; unlisted class → soft note (advisory) unless `strict` → refused, no side effect. Pinned by `tests/artifact-bus.mjs` (66/66). Seed-only. **Deferred**: AB4 shared consent shell (only install is modal in v1; ingest/compose silent — extract on a 2nd modal class), AB6 self-description reporting of `accepts` (`rwa doc --json`/`describe()` — the gate enforces regardless), rwa-onto-rwa compose (the prize), and the `transform` class. Regenerate references after a seed change.
