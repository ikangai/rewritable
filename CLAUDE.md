...
- **Inline manual edit (direct, no-LLM block editing / double-click to edit a block)** → `seeds/rewritable.html` (the `serializeLeafSafe`/`commitInlineEdit`/`handleMountDblClick` block beside the lens-anchor code, + the `dblclick` registration in `renderDoc`) AND `docs/specs/rwa-lens-spec.md` §5.1 (the single-click-anchor vs double-click-edit boundary note) AND `docs/plans/2026-06-08-inline-manual-edit-design.md` (design). Pinned by `tests/inline-edit.mjs` (40/40) + active-view inertness in `tests/view.mjs`. A separate direct-manipulation `edit-surface` (actor `user:edit-surface`) that rides the existing non-agent commit path (`runtimeApplyEnvelope`); it is NOT a lens mode. Editable set = leaf-text members of `ANCHORABLE_TAGS` (`p`/`h1-6`/`blockquote`/`li`/`td`). Regenerate references after a seed change.
