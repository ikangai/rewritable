...
| `frozen_zone_violation` | An edit's `find` or `replace` contains a reserved marker substring or `data-rwa-frozen`. **Runtime only** — see the next row for the CLI/service split. |
| `reserved_substring` | **CLI and service only.** `cli/src/apply-edits.mjs` deliberately splits the runtime's single `frozen_zone_violation` in two: `reserved_substring` when `find`/`replace` *contains* a reserved marker, and `frozen_zone_violation` when an edit's range *overlaps* a marker-form zone. The split is intentional (the CLI's dispatch is keyed on the distinct codes) but this code was absent from this table, so a caller reading only the spec would misclassify a CLI-path violation as an unknown error. |
