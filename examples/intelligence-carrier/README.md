...
- It carries a structured **`recommended_model` / `recommended_backend`** on the envelope (intelligence/0.2 **I-A**). On activation (the “AI” chip → Activate) the runtime offers to apply it to your session behind a one-line consent — never auto-applied, only `rwa_model`/`rwa_backend`, never a base-URL or the API key. The field rides *outside* the signed `agent`, so it was added without re-signing (the signature still verifies).
