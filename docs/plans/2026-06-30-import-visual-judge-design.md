...
- **Eager rasterization (perf, accepted).** `buildFidelityCompare` rasterizes *every* PDF page to a canvas up front, though the compare shows one page at a time and opens on the worst. Fine for typical docs (invoices, a few pages); for a very large PDF that's N canvases for a view the user may not open. A lazy per-page rasterization (render the shown page on demand, cache it) is the follow-up — deferred to avoid re-churning the just-verified UI for a Minor perf gain.
- **Chromium-only judge** (foreignObject taint in Firefox) — see the `rasterizeImport` invariant note in the seed; degrades gracefully.
- **Offline metric is a floor** (substring coverage + U+FFFD garble) — see the sensitivity note in `cli/src/import-fidelity.mjs`.
