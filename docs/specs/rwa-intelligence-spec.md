...
Far shorter than `0.1`'s §6, because most of that list shipped. **I-A, I-C, I-D, and I-E are now built too**; only **I-B** remains — and it is recommended *against* (see below).
...
- **I-C — Intelligence as a first-class authoring object. BUILT** (as a CLI scaffold, reusing `rwa-agent/1` — the signed canon is **not** forked). `rwa intelligence new <role> --prompt "…" [--model …] [--backend …] [--affinity kind,…] [--vault ns,…] [--out f]` mints an Ed25519-signed `rwa-agent/1` record and scaffolds a carrier rewritable (a `skill-host` holding the record + a self-describing card); the **private key** is written to a sibling `.key.json`, kept out of the carrier — needed to re-sign updates under the same identity. `cli/src/intelligence.mjs`, pinned by `cli/tests/intelligence.test.mjs`. Deliberately **not** a new wire-type or `PRODUCT_KIND`: first-class at the *authoring/identity* level, not a second registry (the heavy `intelligence/1` fork was rejected — it would duplicate the agent canon across its mirror sites for little gain).
- **I-D — Kind-affinity. BUILT (advisory, not a hard gate).** A role may declare an unsigned `affinity` (the document kinds it suits); activating or advising it on a mismatched `PRODUCT_KIND` shows a **non-blocking warning** and the panel surfaces the affinity — never refused (consistent with §4's soft note; can't lock you out of your own document). Seed: `getAffinity` / `affinityWarning`; pinned by `tests/intelligence-affinity.mjs`. A *hard* gate was rejected (it reverses §4 and risks locking a user out).
