...
So `intelligence/0.2` is **almost entirely convention over built parts**: the carrier is a `skill-host` rwa (a kind that already owns the `#rwa-agents` zone, the install dialog, and `describe()` surfacing); the overlay is the v0.9 role; transport, travel, revert, and legibility are all shipped. Exactly **two surfaces are new, and both are thin**: **(1)** a **file-drop bridge** that extracts a carrier's signed record and routes it to the existing `runtime.agents.install` — the record parse and the install both already exist, so this only wires them together; and **(2)** the optional structured model-recommendation channel (§6, I-A). Nothing else needs building.

A worked reference carrier — [`examples/intelligence-carrier/concise-editor.html`](../../examples/intelligence-carrier/concise-editor.html) — demonstrates this end-to-end: a real `skill-host` rwa carrying one genuinely-signed `concise-editor` role in its frozen `#rwa-agents` zone plus the self-describing card. It is verified (signature + `validateAgentInstall` + `parseAgentZone`) and **booted** (the runtime lists the role `verified:true`, `describe()` surfaces it as a `kind:'agent'` affordance, and it activates). It anchors this spec the way `hello.html` anchors the core spec; the only piece it cannot show is the unbuilt file-drop gesture above.

---

## 6. Forward design — quarantined, each blocked on its actual substrate

Far shorter than `0.1`'s §6, because most of that list shipped. What remains:

- **I-A — Structured model recommendation.** Today an intelligence can only carry a model hint as a human-read note that the user applies via Settings. A buildable next step: an **additive, non-secret** `recommended_model` / `recommended_backend` field on `rwa-agent/1` that, on activate, the runtime *offers to apply* to `sessionStorage` behind a one-line consent. This honors the invariant — the file still stores no credential and no model is auto-applied; a recommendation is a suggestion, not stored session state. *Blocked on:* an additive schema field + a consent-to-apply affordance. Small; nothing hard.
...
- **I-E — Blended / multiple active overlays.** Today exactly one role is active; the multi-agent layer is data-model messaging (`runtime.agents.message`, `{type,id,from_role,to_role,payload}` on `agents:*`), not prompt-blending. Composing two overlays into one framing is unspecified. *Blocked on:* a precedence/merge model for `system_prompt` framings that doesn't let one overlay silently dominate.
