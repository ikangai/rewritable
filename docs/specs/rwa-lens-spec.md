...
The lens and inline edit surface are available only in **Edit** mode. A rewritable opens in **Document** mode on every page load; Document mode renders the content and any registered render view, but does not attach click-to-edit, click-to-anchor, drag/drop image insertion, paste image insertion, lens history, palette, skin, or model-settings affordances.

The runtime exposes `runtime.mode`, `runtime.setMode(mode)`, and `runtime.on('mode', cb)` for `document`, `edit`, `skills`, and `actions`. Switching into Edit exits any active render view and renders the source document view before edit handlers attach. Switching away from Edit exits inline edit, releases lens anchors, and hides transient edit chrome without committing changes by itself. Mode switches are in-memory only and are rejected while the modify mutex is held.

The four modes remain the runtime's mode model, but the chrome presents them as an **Edit toggle plus a single ⋯ overflow menu**, not a four-tab bar: Document is the quiet default, the Edit toggle lights the editing affordances, and *Activity* (the `actions` panel) and *Skills* (the `skills` panel, shown only on a skill-host) live in the menu alongside the non-mode actions (settings, skins, share, *what is this?*). ⌘S surfaces in the chrome only when the document is dirty. This is presentation only — `runtime.setMode`, the `'mode'` event, and the `data-rwa-mode` gating are unchanged. Design: `docs/plans/2026-06-24-chrome-edit-toggle-design.md`.
...
In Edit mode, anchoring is primarily a click — click a block, the lens moves to sit below it. Drag-and-drop on the lens itself is supported as the explicit alternative gesture. Both produce the same end state.
...
**Leaf-click edit vs container-click anchor.** In Edit mode, a click on an editable leaf text block (`p`/`h1`-`h6`/`blockquote`/`li`/`td`) enters *inline manual edit* immediately: the block becomes `contenteditable`, the user edits in place, and Enter/blur commits through the non-agent path (`actor:'user:edit-surface'`) with **no model call**. Pointer-down opens the edit early enough for the browser to place the caret where the user clicked, so the surface behaves like WYSIWYG page text. A click on a non-editable anchorable container (for example `figure`, `pre`, `aside`, or a table container) still anchors the lens. Double-click remains a compatibility path for inline edit, not the primary gesture. This is not a lens mode and uses none of the lens chrome; it is documented in `docs/plans/2026-06-08-inline-manual-edit-design.md`.

**The working block.** The editable leaf the caret is in is *the block you're working on*, and it is the single source of truth for three things that were previously tracked separately and could drift apart: the inline-edit `contenteditable` target, the black anchor outline, and the ⌘K / anchored-command target. Entering an inline edit anchors the lens to that block and outlines it; exiting clears both. No other path sets a leaf's outline — `container-click` anchoring (above) fires only for the non-editable containers, which have no inline edit. Switching to another editable leaf is **one click**: if the current block is unchanged the runtime exits and re-enters synchronously, so the outline follows in the same gesture; if the current block was edited, the runtime records the target's *ordinal*, commits the current block (which re-renders), then re-resolves the target by that ordinal and enters it — the ordinal is stable because committing a leaf changes its text, not the block order. Drag-selecting opens the inline edit on the block under the pointer, so the block you select in is the working block and carries both the outline and the selection formatting bubble (below). This removes a class of outline/cursor races where the highlight could land one block away from the caret. Design: `docs/plans/2026-06-24-working-block-outline-design.md`.

**Dual mode inside inline edit.** The inline manual edit itself carries the lens's slash discrimination. Direct text (the default, above) commits by hand with no model call. Text beginning with `/` flips the session into *prompt mode*: the typed `/…` is an instruction about the block, never content. **Enter** runs it through the same block-scoped agent path as an anchored slash command (§5.4) — same envelope synthesis, same retry budget, one undo frame; the agent edits the block's *committed* source, so clearing a block to type a prompt never destroys it. **Esc** demotes the prompt to literal text for the remainder of the edit session (a second Esc reverts as always); **blur** discards the prompt without committing or calling the model — click-away is not consent to a model call. The `/` discriminator is recognized at any **word boundary** inside the block — block start, after leading whitespace, or after any interior whitespace — so a command can be typed mid-block (`fix this /shorten it`), and the **last** such `/` wins, so a trailing command is what reaches the model. `\/` escapes a literal slash, and mid-word slashes (`http://`, `/etc/hosts`) are not commands. Detection runs on the block's safe serialization — what a commit would actually write — not raw `innerHTML`, so the mode the user sees matches what a commit would do. While a command is engaged the runtime paints a Notion-style mode cue: only the `/…suffix` is coloured (an inline accent span, `data-rwa-cmd-text`) and the block carries a faint accent tint (`data-rwa-cmd`), leaving the content before the command visibly normal. The instruction sent to the model is the suffix after the winning `/`; the `/command` text is never committed (running it restores the block's committed source, then applies the agent's block-scoped edit), and a lone `/` is a no-op. Prompt mode lives entirely inside the inline-edit surface. Design: `docs/plans/2026-06-09-inline-lens-dual-mode-design.md` (Increment 1, the leading-slash discriminator; the word-boundary mid-block `/` and the colour cue are 2026-06-24 refinements).

**Selection formatting bubble, commands, and voice input.** In Edit mode, selecting text inside one editable leaf block opens a small runtime-only floating bubble above the selection. It exposes direct, deterministic formatting controls alongside a typed/voice command input. The controls split by scope, matching what `rwa-edit/1` can already express:

- **Inline** (wrap the selected occurrence): **Bold** `<strong>`, **Italic** `<em>`, **Underline** `<u>`, **Strikethrough** `<s>`, inline **Code** `<code>`, **Link** `<a href>` (the URL is scheme-sanitized against the same allowlist as import — `http`/`https`/`mailto`/`tel`/relative), and **text colour** (a swatch palette → `<span style="color:…">`).
- **Block** (retag or restyle the whole containing leaf): a heading control (Paragraph / H1 / H2 / H3, which retags the block) and **alignment** (left / center / right via `style="text-align:…"` on the block).

Every control compiles locally to an `rwa-edit/1` envelope and commits through the non-agent path (`surface:'selection-edit'`, `actor:'user:selection-command'`) — instant, no model call, one ⌘Z per action; frozen-zone and structural guards still apply because the apply pipeline is the schema. A control reflects active state when the selection already carries that format. The command input is the same parser as before and can be typed or supplied through browser speech recognition (voice is only an input method, not a separate surface); deterministic phrasings (`make it bold`, `italic`, `inline code`) compile locally, and the model is reachable only as the explicit fallback. The runtime maps the selected text back to the selected occurrence in the source block, so repeated words in the same block target the occurrence the user selected. Unsupported commands fail visibly instead of guessing or calling a model implicitly. The bubble brings the surface to feature parity with visual editors (heyhtml.com) while keeping the `/`-prompt LLM as the differentiator. Design: `docs/plans/2026-06-24-wysiwyg-editing-parity-design.md`.
...
13. The working block — the editable leaf the caret is in — is the single source of truth for the inline-edit target, the anchor outline, and the ⌘K / anchored-command target; they are set together on edit entry and cleared together on exit, and a block switch moves all three atomically. No second path outlines or anchors an editable leaf (§5.1).

---

*Spec version 0.12 — folds the shipped WYSIWYG editing surface into the normative model: the working-block single-source-of-truth (§5.1, Invariant 13); the selection formatting bubble (§5.1 — inline Bold/Italic/Underline/Strikethrough/Code/Link/colour plus block heading and alignment, all on the non-agent `selection-edit` commit path); the word-boundary mid-block `/` discriminator with the Notion-style colour cue (§5.1); and the Edit-toggle + ⋯-menu chrome IA (§4.5). Companion to re-write-able core spec v0.15 and rwa-edit-spec.md v1.6.*
