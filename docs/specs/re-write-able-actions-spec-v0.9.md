# re-write-able actions skill layer — v0.9 specification

This document is the numbered **v0.9** of the actions/skill/permission/Worker-mode spec: the thirteen items that v0.8 §11 deferred. It **extends** v0.8 and **never breaks** it — every wire format, permission grammar string, invariant number (18–20), and bridge error vocabulary in v0.8 stays byte-stable, and all additions are forward-compatible. v0.8 §11 is the canonical source list; each deferred line is turned into a normative definition the build was lifted from. **All thirteen items (I1–I13) are built, browser-proven, and shipped** (seed + rewritable.ikangai.com, CLI `rewritable@0.11.0`). The build-status paragraph below records the per-item refinements made during the build and is authoritative wherever it refines the original draft text; section ordering and the parameter constants (Argon2 cost, idle timeouts, caps) follow the build. (This file was promoted from `re-write-able-actions-spec-v0.9-open-items.md` on 2026-06-26 — same content, numbered.)
...
**Build status (2026-06-22).** The P0 tier is shipped: **I10** (`runtimeReviewSkill` update delta + the dialog's added/removed diff and re-affirmation button — seed; jsdom `tests/skill-install.mjs` + Chromium `tests/skill-exec-probe.mjs`) and **I11** (`rwa install` — `cli/src/install.mjs`, `cli/tests/install.test.mjs`, cross-tool browser-verified). **I1 is partially built — I1a** (the `bus:` grammar + the Worker *publish* bridge) shipped: grammar/prose/compound mirrored seed↔CLI, `RUNTIME.bus.publish` gated by `_skBusAllowed`, browser-proven (a skill publishes on its declared topic; an undeclared topic is `bus_topic_denied`). **I1b** (skill-side *subscribe*) is now BUILT (after I2): the prologue's `RUNTIME.bus.subscribe(topic, cb)` bridges `bridge:bus:subscribe`, the main thread gates `_skBusAllowed` then forwards each `runtimeBusSubscribe` envelope (filtered through `_skBusMessageAllowed`, true today — the I12 peer-allowlist hook) as a `bus:message` to the Worker, which dispatches to the registered cb; subscriptions are torn down when the invoke settles (`busSubs` in `finish`); the 5s timeout bounds the subscribe CALL, not the subscription. Browser-proven `skill-exec-probe` 51/0: a tool subscribes to its declared topic and receives a foreign-sender message during its invoke; an undeclared topic → `bus_topic_denied`. **I6** (P2, signed-skill marketplace) BUILT across three components. **Service** (`service/server.js`, vendoring `lib/skill-manifest.mjs`): `POST /skills/publish` (verify + `validateInstall` → store `{envelope, metadata}` under `DATA_DIR/skills/`), `GET /skills/index` (paginated/filterable, sorted, nosniff + CORS-open read), `GET /skills/index/:id` (full envelope; 404/410), `POST /skills/revoke/:id` (Ed25519 over `'REVOKE:'+id+ts` by the author key, permanent), `POST /skills/report/:id` (rate-limited queue, no auto-block). **CLI** `rwa skill publish <file>` (`cli/src/skill-publish.mjs`): POST a pre-signed envelope, local fail-fast gate, injected transport. **Seed**: `runtime.discoverSkills` (GET index) + `runtime.fetchSkillFromIndex` (GET detail → **client-side WebCrypto verify** before install — never trust the index's `verified`) + TOFU author identity in the install dialog (`_skFingerprint` = sha256(pubkey)[:16] + the per-author install count from I5's `rwa_sources`: "first time seeing this author" vs "trusted, N installs"). Also re-vendored `service/lib/{skill-manifest,identity,seed}.mjs` to cli/src (a 0.8.0 `seed.mjs` drift the cmp gate caught). TDD: `service/tests/skills.test.mjs` 6/0, full service 77/0; `cli/tests/skill-publish.test.mjs` 5/0; seed `skill-install` 83/0 (TOFU). Browser-proven `skill-exec-probe` 54/0: discover → fetch → real client-side verify → install-from-index. The index informs; install-time human review (the dialog) stays the trust anchor (Shape B). **I3 + I4** (P1) shipped together (they share the bridge+gate skeleton): the `fsa:` (scoped OPFS) and `idb:` (scoped IndexedDB store) tiers — grammar mirrored seed↔CLI, `RUNTIME.fs`/`RUNTIME.db` Worker proxies gated by `_skFsAllowed`/`_skIdbAllowed`, closed error vocab. Browser-proven (`skill-exec-probe` 23/0): idb round-trip + denial at `file://`; fsa gate at `file://` + a real OPFS round-trip over `http://localhost`. **Refinement:** I3 uses a **full-path** model (the skill addresses `data/x` and it must fall under a declared `fsa:` scope) rather than §6's scope-prepend example — unambiguous with multiple scopes; §6 below describes scope-prepend, the code is full-path. **I5** (P1) shipped in two parts: the **homoglyph block** (NFKC + a curated cross-script confusables skeleton; a signed homoglyph of a different author's installed skill → `lookalike_skeleton_blocked`) and **name_history** (per-author `rwa_sources` in IDB, reconciled at boot from the in-file manifests; the dialog surfaces a same-key rename). Seed + CLI mirror; browser-proven `skill-exec-probe` 27/0 incl. a name_history round-trip against **real IndexedDB**. Two refinements vs. the §4 text below: (1) the install block fires on `skeleton ≤1 AND skeleton < normalized-Levenshtein` — i.e. confusable folding must have *collapsed* a real byte difference — so an honest ASCII near-miss (skeleton == Levenshtein) still only WARNS, reconciling §4's "skeleton ≤1 blocks" with its own acceptance "ASCII exact name, diff key → warning, install allowed"; (2) `first_seen_at_name` dates are **best-effort** — the deterministic frozen-zone bytes carry no timestamps, so an IDB-cleared reload reconciles names with the reconcile time as the date (identity itself is always the key, never the date). The CLI mirror of name_history is registry-derived + dateless (no persistent IDB per-invocation). **I12** (P2, multi-agent orchestration) BUILT on v0.8 + I1a (it needs neither I1b nor I8: the bus piece is the data-model message shape; the request→response choreography is the conductor's job over the doc-side bus). A signed `rwa-agent/1` record (role + system_prompt + vault_namespace_set, NO code) installs into a SECOND frozen zone `#rwa-agents` (coexists with `#rwa-skills`; `runtimeRegionCommit` already commits multiple regions atomically). Agent signing canon mirrored seed↔CLI; boot re-verify; `runtime.agents.{list,active,setActive,install,uninstall,message,showInstallDialog}`; no agent active by default; a tampered (unverified) agent installs `verified:false` but can't activate (`unverified_agent`). When a role is active: `getActiveActor()` → `agents:${role}` (history attribution), `modify()`'s system prompt swaps the role FRAMING while keeping the shared tool rules, and `invokeSkill(…,{agentRole})` gates the vault bridge on the AGENT's `vault_namespace_set` (the role NARROWS the skill). Inter-agent bus message shape `{type,id,from_role,to_role,payload}` on `agents:*` (data-model only). SD-04 extended: `parseAgentZone` (one copy in `skill-manifest.mjs`, imported by the oracle + the CLI mirror; `AFFORDANCE_KINDS += 'agent'`) surfaces installed agents as `{agentId, kind:'agent', name:role, verified, provenance:'installed'}` affordances; the seed's live `describe()` emits the same shape. Install dialog: a dedicated agent-record dialog (role, author key, vault namespaces, ≤200-char prompt preview; install offered only for a verified+gate-passing record) + format dispatch. Built across A (registry) + B (role binding) + C1 (bus shape) + C2 (SD-04 + dialog); seed↔CLI mirror; browser-proven `skill-exec-probe` 32/0 incl. role-scoped vault through a real Worker (in-set round-trips, out-of-set denied). **I8** (P1, hook kind) BUILT in two phases. The `hook:<event>` tier (event ∈ {on-commit, on-open, on-mode-change}, exact enum, unknown event → `unknown_permission_tier`); a hook is compute-only (only hook: perms, else `compute_with_permissions`) and must be signed+verified (`unsigned_capability`) since it runs autonomously; mirrored seed↔CLI + prose. Firing is centralized in `emitRuntimeEvent`: the seed's `'modify'` event (an edit commit, carrying `instruction`/`lensMeta`) maps to `on-commit`, `'mode'` to `on-mode-change` (with the prior mode tracked for `previous`); `on-open` fires at boot. Fire-and-forget (never blocks/throws into the emitter), deterministic skillId order, re-entrancy-guarded (`activeHooks`), pure-compute Workers (`bridged:isTool` is false for hooks → no network/vault), every run logged to the new IDB store `rwa_hook_log` (pruned >1000/>30d at boot; `runtime.hookLog()` reads it). Browser-proven `skill-exec-probe` 35/0: a hook fires on a real `setMode`, runs in a real Worker, returns `{saw:'on-mode-change', hasFetch:'undefined'}` (compute-only), and the result lands in the audit log. **Note:** on-commit is the seed's edit-commit (`'modify'` event), not the ⌘S file-save (`'commit'` event); on-open is fire-and-forget (not boot-blocking) so a hook can't slow first paint. **I7** (P1, view / edit-surface skills) BUILT. New kinds `{view, edit-surface}` are zero-capability DOM authors: `validateInstall` rejects any permission (`output_skill_with_permissions`) and requires a matching typed output (`view`→`output.kind:'html-render'`, `edit-surface`→`'dom-transform'`, else `invalid_output_kind`); unsigned is OK (like compute); mirrored seed↔CLI. **Execution model (a coherent simplification of §8's Worker render/transform bridge):** both are pure-compute Workers whose RETURN VALUE is the output, validated main-side — a **view** skill's `run({doc,ctx})` returns HTML, validated by the existing first-party `validateViewOutput` (no `<script>`, no reserved ids) and activated as a SNAPSHOT overlay (`runtimeSetView` accepts an installed skillId; async-invoke → validate → render; idempotent; a view never commits — auto-refresh-on-change is the deferred `observe` opt-in); an **edit-surface** skill's `run(input)` returns an `rwa-edit/1` envelope, applied via `runtime.invokeEditSurface` through the existing `runtimeApplyEnvelope` path (so the *apply pipeline is the schema* — frozen-zone + structural guards, one ⌘Z, actor `skill:transform:<id8>`); a non-envelope return is rejected `invalid_transform_output`. Self-description already surfaces the kinds (`parseSkillZone` + `AFFORDANCE_KINDS`). Browser-proven `skill-exec-probe` 40/0: a view renders in a real Worker (compute-only, fetch undefined) and applies to the mount; edit-surface's malformed-output rejection fires after a real invoke. **I2** (P1, compute-Worker pool) BUILT, seed-only (the CLI has no Workers). **DISABLED BY DEFAULT** — only `runtimeInvokeSkill(id, input, {pooling:'enabled'})` on a COMPUTE skill (no agent-role) takes the warm path via `_skPooledInvoke`; every other invoke rides the byte-unchanged spawn→invoke→terminate path (a 1-line guard). Pooled Workers are keyed by `_skCodeHash(skillId+code)` (a code change → `_skEvictPool`, also fired on install/update/uninstall), compute-only (bridgeless, re-init'd with a fresh identity_tag + the 5s timeout PER invocation), returned to the pool on success and terminated (never pooled) on timeout/error. Bounded by `SKILL_POOL_CAP = min(4, hardwareConcurrency)` (`_skEnforcePoolCap` evicts oldest-idle) + a 60s idle sweep (`_skPoolEvictIdle`, every 30s); drained on `pagehide` (`_skPoolShutdown`: shutdown handshake + 500ms grace + terminate; the prologue replies `shutdown_ack`). `runtime.poolStats()` for observability. Browser-proven `skill-exec-probe` 48/0: a pooled Worker is reused (same closure id, invoke counter increments), compute-only, disabled-by-default (fresh each with no hint), code-change evicts the pool, a tool is never pooled, and `pagehide` drains it. **Note:** idle-eviction (60s) + cap are wired (logic + the cap bound is unit-checked) but not browser-timing-asserted; statelessness is the author's responsibility (no global reset between invokes — Inv 25). **I13** (P2, account-linked identity) BUILT to the v0.9 scope — **offline export/import** (escrow + the account service deferred to v1, per §14's "SHOULD NOT require an account service in v0.9"). `runtime.vault.export(passphrase, namespaces)` re-encrypts selected namespaces under a SEPARATE transport passphrase (PBKDF2-200k + AES-256-GCM, per-namespace salt + check) into a self-contained version-tagged `rwa-vault-export/1`; `runtime.vault.import(exportObj, passphrase, {overwrite})` validates (`account_export_malformed`), fails on a wrong passphrase BEFORE any write (`vault_decrypt_failed`), re-wraps under the LOCAL vault key, skips existing entries unless `overwrite`, reports `containerMismatch`. self-description gains a live-only `accountIdentity` (null by default; never stamped, never exposed to skill code). No new crypto (reuses the vault's PBKDF2+AES-GCM); seed-only, jsdom-faithful. Pinned by `tests/vault-export.mjs` 10/0 (a true cross-container round-trip: A exports → B with its own empty vault imports → `vault.get` returns A's original; ciphertext≠plaintext; wrong-passphrase no-partial; malformed rejected; re-import no-clobber). **I9** (Argon2id vault KDF) BUILT — and the blocker dissolved on a fresh look. The staged plan (revise the frozen CSP + vendor WASM argon2) would have **violated** Inv 26/44 + §0 ("CSP unchanged"); the frozen `script-src 'unsafe-inline' blob:` has no `'wasm-unsafe-eval'`, so WASM is genuinely blocked. Resolution: a **pure-JS Argon2id inline module** (the normative contract already admits "single-file WASM/**inline module** … CSP-safe `'unsafe-inline'`"; line 796's CSP clause is conditional) — it needs no WASM and no CSP change, so Inv 26/44/18 are **held, not breached**. Vendored from `@noble/hashes` v2.2.0 via `tools/vendor-argon2.mjs` (esbuild iife/min, ~13 KB), inlined as the string `ARGON2_SRC` between `// rwa:argon2:begin/end` markers and RFC-9106-pinned. `_vaultDeriveKey(passphrase, saltB64, kdfVersion=0)`: 0=PBKDF2-200k (v0.8 + I13 transport callers default here, byte-unchanged), 1=Argon2id(m=64 MiB,t=3,p=4)→AES-GCM-256. The ~1.5 s derive runs in a transient `blob:` Worker (`_argon2idViaWorker`/`_argon2idHash`) so unlock never freezes; a no-Worker host (jsdom) uses a sync fallback. `runtimeVaultUnlock(passphrase, {targetKdfVersion?})` migrates atomically (re-encrypt check + every entry under the new key → ONE record write; `_vaultKey`/`_vaultRec` assigned only after the put resolves → a mid-migration failure leaves the vault locked + the stored record intact, Inv 43). New vaults default to v1; `check==null` covers new-default + auto-migrate-on-empty; `kdf_version>1`→`vault_unknown_kdf_version` (no silent downgrade). A skill-host settings button ("Upgrade to Argon2id") triggers `{targetKdfVersion:1}`. Seed-only (no CLI/service vault mirror). Pinned by `tests/vault-kdf.mjs` 22/0 (RFC-9106 vector against the seed bytes + new-default/forward-compat/migration/atomicity/auto-empty/unknown-version/I13-PBKDF2-regression); I13 `tests/vault-export.mjs` + `tests/vault.mjs` updated with the jsdom Argon2id shim (`tests/lib/argon2-fallback.mjs`). The `blob:` Worker path is browser-verified. **All 13 v0.9 open items are now BUILT.**

Both honor the §0 constraints; none lifts the Shape B ceiling. **All 13 items (I1–I13) are now BUILT** — the sections below are the as-specified contract each was built against; refinements made during the build are noted in the build-status paragraph above.
...
| I1 | Bus permission tier | **P1 — ✅ BUILT (I1a publish + I1b subscribe)** | M |
| I3 | `fsa:` tier | **P1 — ✅ BUILT** | M |
| I4 | `idb:` tier | **P1 — ✅ BUILT** | M |
| I7 | view / edit-surface skills | **P1 — ✅ BUILT** | M |
| I8 | `hook` kind | **P1 — ✅ BUILT** | M |
| I2 | Worker pool & lifecycle | **P1 — ✅ BUILT** | M |
| I6 | Signed-skill marketplace | **P2 — ✅ BUILT** | L |
| I12 | Multi-agent orchestration | **P2 — ✅ BUILT** | L |
| I9 | Argon2id vault KDF | **P2** | M |
| I13 | Account-linked identity | **P2 — ✅ BUILT (export/import; escrow→v1)** | M |
...
> **BUILT (2026-06-23) — pure-JS, NOT WASM.** This section's "a vendored single-file WASM is required" framing (Motivation/Open questions) is **superseded**: WASM needs `'wasm-unsafe-eval'`, which the frozen CSP lacks and Inv 26/44 forbid adding. The build uses a **pure-JS Argon2id inline module** (`@noble/hashes` v2.2.0, bundled by `tools/vendor-argon2.mjs`, inlined as `ARGON2_SRC`), run in a `blob:` Worker — no WASM, no CSP change, every normative MUST satisfied. See the build-status paragraph (top of doc) and `docs/plans/2026-06-23-i9-argon2id-kdf-design.md`. The Normative contract below holds as written (it already admitted "WASM/**inline module**").
